...
# CUDA_VISIBLE_DEVICES='4' python3 run.py --do_eval --model ./trained_model/ --output_dir ./eval_output --per_device_eval_batch_size=256
...
def main():
    ...
    argp.add_argument('--task', type=str,default="nli",
    ...
    if args.dataset=='anli':
        count=0
        for i in tqdm(dataset['train_r1']):
            aug_list=augmenter.augment(i['premise'])
            for k in aug_list:
                data_dict["label"].append(i['label'])
                data_dict["premise"].append(k)
                data_dict["hypothesis"].append(i['hypothesis'])
            data_dict["label"].append(i['label'])
            data_dict["premise"].append(i['premise'])
            data_dict["hypothesis"].append(i['hypothesis'])
        dataset_extend_train = Dataset.from_dict(data_dict)
        f=open('./ani_full_extend.pkl','wb')  
        pickle.dump(dataset_extend_train,f) 
        f.close()
    ...
    if args.dataset=='anli':
        f1 = open("./ani_full_extend.pkl", 'rb')
        dataset_extend_train = pickle.load(f1)
        f1.close()
        print("here")
    if args.dataset=='snli':
        f1 = open("./data_full_extend.pkl", 'rb')
        dataset_extend_train = pickle.load(f1)
        f1.close()
    ...
    if args.dataset=='snli':
        if training_args.do_train:
            train_dataset =dataset_extend_train
